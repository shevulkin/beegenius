<?php
// Список постів блогу, з опційним фільтром за тегом (?tag=slug) і пошуком за заголовком (?q=).
$posts = [];
$allTags = [];
...
try {
    $pdo = db_connect();
    $allTags = get_tags_for_type($pdo, 'blog');

    $params = [];
    $conditions = ["a.type = 'blog'", "a.status = 'published'"];
    $join = '';
    if ($activeTag !== '') {
    ...
    error_log('[blog_list] DB error: ' . $e->getMessage());
}

$pageTitle = 'Блог — Bee Genius';
$metaDescription = 'Щоденник пасічника Валентина: нотатки про вуликах, сезон і життя поруч з бджолами.';
$active = 'blog';
...
<section class="section">
  <div class="section-head">
    ...
    <h1 class="serif">Щоденник пасічника</h1>
    <p>Нотатки про вуликах, сезон і про життя поруч з бджолами.</p>
  </div>
  <form class="search-form" method="get" action="<?= BASE_PATH ?>/blog">
    ...
    <?php if ($q !== ''): ?><a class="search-clear" href="<?= BASE_PATH ?>/blog<?= $activeTag !== '' ? '?tag=' . urlencode($activeTag) : '' ?>">Скинути</a><?php endif; ?>
  </form>
  <?php if (!empty($allTags)): ?>
    <div class="tag-filter">
      <a class="tag-chip <?= $activeTag === '' ? 'active' : '' ?>" href="<?= BASE_PATH ?>/blog<?= $q !== '' ? '?q=' . urlencode($q) : '' ?>">Усі</a>
      <?php foreach ($allTags as $t): ?>
        <a class="tag-chip <?= $activeTag === $t['slug'] ? 'active' : '' ?>" href="<?= BASE_PATH ?>/blog?tag=<?= urlencode($t['slug']) ?><?= $q !== '' ? '&q=' . urlencode($q) : '' ?>">#<?= htmlspecialchars($t['name']) ?></a>
      <?php endforeach; ?>
    </div>
  <?php endif; ?>
  <div class="card-grid">
  ...
  <?php if (empty($posts)): ?>
    <p class="empty"><?= ($activeTag !== '' || $q !== '') ? 'Нічого не знайдено за цим фільтром.' : 'Поки що немає жодного посту.' ?></p>
  <?php endif; ?>
  <?= render_pagination($page, $totalPages, BASE_PATH . '/blog', ['tag' => $activeTag, 'q' => $q]) ?>
</section>
