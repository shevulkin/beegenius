...
// Лише bare-ім'я файлу в межах теки — без шляху й без "..", щоб виключити
// вихід за межі uploads/articles. Символи самої назви (кирилиця, пробіли
// тощо) дозволені — старі фото могли потрапити сюди ще до того, як
// завантаження почало генерувати випадкові латинські імена; остаточну
// гарантію "файл справді в межах теки" однаково дає realpath()-перевірка нижче.
if ($name === '' || str_contains($name, '/') || str_contains($name, '\\') || str_contains($name, "\0") || str_contains($name, '..')) {
